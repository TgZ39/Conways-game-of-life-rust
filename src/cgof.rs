...
impl Board {
    ...
    pub fn get_cell(&self, x: u32, y: u32) -> Option<Cell> {
        match self.cells.get((self.length*y+x) as usize) {
    ...
    pub fn set_cell(&mut self, x: u32, y: u32, alive: Cell) {
        match self.cells.get((self.length*y+x) as usize) {
            Some(_) => self.cells[(self.length*y+x) as usize] = alive,
            None => {},
        }
    }
        ...
        for x_offset in [-1, 0, 1] {
            ...
            for y_offset in [-1, 0, 1] {
                if y+y_offset < 0
                    ...
                    || self.get_cell((x+x_offset) as u32, (y+y_offset) as u32).is_none()
                    || self.get_cell((x+x_offset) as u32, (y+y_offset) as u32).unwrap() == Dead {
                    continue
                }
                count += 1;
            }
